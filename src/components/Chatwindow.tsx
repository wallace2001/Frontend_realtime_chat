...
import { GET_MESSAGES_FOR_CHATROOM_M } from "../graphql/mutations/GetMessagesForChatroom"
...
export const ChatWindow = () => {
  ...
  const [_, setLiveUsers] = useState<User[]>([])
  ...
  const [getMessagesForChatroom] = useMutation(GET_MESSAGES_FOR_CHATROOM_M);
  const chatroomId = parseInt(id!)

  const [nameRoom, setNameRoom] = useState<string>('');

  const { data: chatroom } = useQuery<GetChatroomsForUserQuery>(
  ...
  useEffect(() => {
    if (chatroom?.getChatroomsForUser) {
      const selectedRoom = chatroom.getChatroomsForUser.find(c => parseInt(c.id!) === chatroomId);
  
      if (selectedRoom?.id) {
        setNameRoom(selectedRoom.name!);
      }
    }
  }, [chatroom?.getChatroomsForUser]);

  const updateMessagesRoom = async () => {
    await getMessagesForChatroom({ variables: { chatroomId } })
      .then((response) => {
        setMessages(response.data.getMessagesForChatroom)
      })
      .catch((error) => {
  ...
  const handleSendMessage = async () => {
    ...
    if (isEmpty(messageContent)) return;
    await sendMessage({
      variables: {
        chatroomId: chatroomId,
  ...
  useEffect(() => {
    if (data?.getMessagesForChatroom) {
      const uniqueMessages = data.getMessagesForChatroom.filter((message, index, self) =>
